				...
				new FigurePageLayoutEditPolicy() {
					...
					protected Command getCreateCommand(CreateRequest request) {
						...
						if (request.getNewObject() instanceof CreateElementAction) {
							...
							return OutlineTreeEditPartFactory.getCreateCommand((ANode) getHost().getModel(),(ANode) request.getNewObject(),constraint.getCopy(), -1, request);
							...
							for (Object obj : c) {
								if (obj instanceof ANode) {
									...
									cmd.add(OutlineTreeEditPartFactory .getCreateCommand((ANode) getHost().getModel(), aObj, constraint.getCopy(), -1, request));
								}
							}
							return cmd;
						}
